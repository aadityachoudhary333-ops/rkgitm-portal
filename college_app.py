...
# Load secrets from .env file (never hardcode credentials!)
load_dotenv()

# --- 1. CONFIGURATION (reads from .env file) ---
...
def get_conn():
    """Returns a new psycopg2 connection. Use as a context manager."""
        ...
        with conn.cursor() as c:
            ...
            c.execute('''
                CREATE TABLE IF NOT EXISTS student_data (
                    ...
                    sub5_name TEXT, sub5_s1 TEXT, sub5_s2 TEXT,
                    profile_pic TEXT
                )
            ''')
            # Seed default accounts (safe: ON CONFLICT DO NOTHING)
            c.execute("""
                INSERT INTO users VALUES ('admin','adminpassword','admin','System Administrator','approved')
                ON CONFLICT (username) DO NOTHING
            """)
            c.execute("""
                INSERT INTO users VALUES ('aaditya','studentpassword','student','Aaditya Chaudhary','approved')
                ON CONFLICT (username) DO NOTHING
            """)
            c.execute("""
                ...
                ('aaditya','2026CS101','Computer Science','4th','aaditya@rkgitm.edu','9876543210',
                 '38','45',
                 'Data Structures','42/50','45/50',
                 'Operating Systems','38/50','Not Uploaded',
                 'Computer Networks','45/50','48/50',
                 'Database Systems','40/50','42/50',
                 'Discrete Math','35/50','Not Uploaded','')
                ON CONFLICT (username) DO NOTHING
            """)
        conn.commit()
...
def main(page: ft.Page):
    ...
    def on_pic_upload(e: ft.FilePickerResultEvent):
        ...
        try:
            ...
            with get_conn() as conn:
                with conn.cursor() as c:
                    c.execute(
                        "UPDATE student_data SET profile_pic=%s WHERE username=%s",
    ...
    def handle_login(e, username_field, password_field, error_text):
        ...
        try:
            with get_conn() as conn:
                with conn.cursor() as c:
                    c.execute(
                        "SELECT role, name, status FROM users WHERE username=%s AND password=%s",
                        (user, password)
                    )
                    record = c.fetchone()
        except Exception as ex:
    ...
    def handle_registration(e, fields, error_text):
        ...
        try:
            with get_conn() as conn:
                with conn.cursor() as c:
                    c.execute(
                        "INSERT INTO users VALUES (%s,%s,'student',%s,'approved')",
    ...
    def show_student_dashboard(username: str, name: str):
        ...
        page.add(
            ft.Container(height=20),
            ft.Row(
                [ft.Text("Student Dashboard", size=24, weight=ft.FontWeight.BOLD),
            ...
            ft.Card(content=ft.Container(content=ft.Column([
                ...
                ft.DataTable(
                    columns=[
                        ft.DataColumn(label=ft.Text("Subject")),
    ...
    def show_admin_dashboard(name: str):
        ...
        def select_student(e, selected_username):
            ...
            if data:
                ...
                curr_subs = [(n1,s1_1,s1_2),(n2,s2_1,s2_2),(n3,s3_1,s3_2),(n4,s4_1,s4_2),(n5,s5_1,s5_2)]
                for i in range(5):
                    subject_inputs[i]["name"].value = curr_subs[i][0]
                    subject_inputs[i]["s1"].value   = curr_subs[i][1]
                    subject_inputs[i]["s2"].value   = curr_subs[i][2]
        ...
        def refresh_table(e=None):
            ...
            with get_conn() as conn:
                with conn.cursor() as c:
                    if query:
                        c.execute('''
                            SELECT users.name, student_data.roll_no, student_data.username
        ...
        page.add(
            ft.Container(height=20),
            ft.Row(
                [ft.Text("Admin Control Panel", size=24, weight=ft.FontWeight.BOLD),
                 ft.IconButton(icon=ft.Icons.LOGOUT, on_click=logout)],
                alignment=ft.MainAxisAlignment.SPACE_BETWEEN
            ),
            ft.Divider(),
            ...
            *subject_ui_rows,
            ...
            ft.ElevatedButton(
                "Start New Semester (Wipe Data)",
                icon=ft.Icons.WARNING,
                on_click=handle_wipe_for_promotion,
                width=550,
